...
demo <- readRDS(here::here("data/processed/demo.rds"))
...
interpolating.years <- expand.grid(MidPeriod = seq(from=min(demo $MidPeriod),
                                                   to = max(demo $MidPeriod), by = 1),
...
thresholds.5y %>% 
  ...
  mutate(old.age = spline(MidPeriod, old.age, xout = MidPeriod)$y) -> thresholds.1y
...
library(dplyr)
source(here::here("code/FunSpline.R"))
demo.pop <- readRDS(here::here("data/processed/demo.pop.rds"))

x <- demo.pop$AgeGrp
y <- cumsum(demo.pop$PopTotal)
